...
int BasicPipelineProgram::InitSkyBox(const char * shaderBasePath)
{
    if (BuildShadersFromFiles(shaderBasePath, "skybox.vertexShader.glsl", "skybox.fragmentShader.glsl") != 0)
    {
        cout << "Failed to build the skybox pipeline program." << endl;
        return 1;
    }
    
    cout << "Successfully built the skybox pipeline program." << endl;
    return 0;
}
