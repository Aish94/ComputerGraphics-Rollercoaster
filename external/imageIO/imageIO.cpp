...
void ImageIO::Rotate()
{
    ...
    for(unsigned int row=0; row < height / 2; row++)
    {
        ...
        for(unsigned int i=0; i<width*bytesPerPixel; i = i+bytesPerPixel)
            for(int j=0;j<bytesPerPixel;j++)
                rowBuffer[i+j] = rowPixels[(width*bytesPerPixel)-bytesPerPixel-i+j];
        
        // copy otherRow to row
        for(unsigned int i=0; i<width*bytesPerPixel; i = i+bytesPerPixel)
            for(int j=0;j<bytesPerPixel;j++)
                rowPixels[i+j] = otherRowPixels[(width*bytesPerPixel)-bytesPerPixel-i+j];
